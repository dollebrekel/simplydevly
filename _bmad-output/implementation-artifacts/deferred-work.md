...
- **Onzichtbare DiffView slokt navigatietoetsen op** — De 31-mei status-bar fix verwijderde alle rendering/sizing van `a.diffView` (en `a.activityFeed`) uit `app.go`, maar liet de message-handlers en key-routing intact. Wanneer een `DiffViewMsg` binnenkomt en `LoadDiff` wordt aangeroepen, wordt de diff view `IsActive()`, waarna de key-router (`app.go:~537-548`) `tab/esc/e/up/down/k/j` afvangt en aan de onzichtbare `diffView.HandleKey` voert — toetsen bereiken de panelManager dan niet meer en de UI lijkt bevroren. Pre-existing (niet veroorzaakt door story tui-layout-fixes-2). Fix: ofwel `LoadDiff`/`HandleKey`-wiring verwijderen, ofwel de diff view weer renderen — render- en input-wiring gesynchroniseerd houden. [internal/tui/app.go:363-366, 537-548]
