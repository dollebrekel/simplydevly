...
// Respects ANTHROPIC_BASE_URL env var for proxy/benchmark compatibility.
...
func (a *Adapter) Query(ctx context.Context, req core.QueryRequest) (<-chan core.StreamEvent, error) {
	...
	if req.Model == "" {
		return nil, fmt.Errorf("anthropic: no model configured — set provider.model in ~/.siply/config.yaml")
	}
