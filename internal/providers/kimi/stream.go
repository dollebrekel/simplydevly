...
// streamParser reads SSE events from a Kimi streaming response.
// Kimi uses the OpenAI-compatible streaming format.
...
// chunkResponse represents a single SSE chunk from Kimi's streaming API.
...
// kimiUsageDetails holds Kimi-specific cache token fields in the usage response.
type kimiUsageDetails struct {
...
func (p *streamParser) readDataLine() (string, error) {
	...
	if err := p.scanner.Err(); err != nil {
		return "", fmt.Errorf("kimi: scanner error: %w", err)
	}
	return "", io.EOF
}
...
func (p *streamParser) handleChunk(data string) (core.StreamEvent, error) {
	...
	if err := json.Unmarshal([]byte(data), &chunk); err != nil {
		return nil, fmt.Errorf("kimi: failed to parse chunk: %w", err)
	}
	...
	if chunk.Usage != nil {
		...
		if chunk.Usage.CacheCreationTokens > 0 {
			usage.CacheCreationInputTokens = chunk.Usage.CacheCreationTokens
		}
		...
		for _, tc := range choice.Delta.ToolCalls {
			...
			if tc.Function.Arguments != "" {
				if acc.JSONBuf.Len()+len(tc.Function.Arguments) > maxToolJSONSize {
					return nil, fmt.Errorf("kimi: tool call input exceeds maximum size (%d bytes)", maxToolJSONSize)
				}
...
func (p *streamParser) emitToolCalls() (core.StreamEvent, error) {
	...
	for _, k := range keys {
		...
		if !json.Valid([]byte(inputJSON)) {
			return nil, fmt.Errorf("kimi: tool call %q produced invalid JSON input", acc.Name)
		}
	...
	if len(events) == 0 {
		return nil, fmt.Errorf("kimi: finish_reason %q received but no tool calls were accumulated", "tool_calls")
	}
	if len(events) > 1 {
