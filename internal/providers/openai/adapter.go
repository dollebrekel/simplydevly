...
func (a *Adapter) Init(ctx context.Context) error {
	if a.credStore == nil {
		return fmt.Errorf("openai: credential store is nil")
	}
	cred, err := a.credStore.GetProvider(ctx, "openai")
	if err != nil {
		return fmt.Errorf("openai: failed to get credentials: %w", err)
	}
	if cred.Value == "" {
...
func (a *Adapter) Health() error {
	...
	if a.client == nil {
		return fmt.Errorf("openai: HTTP client not initialized")
	}
	return nil
}

// Capabilities returns the capability set for GPT models.
...
// Query sends a streaming request to the OpenAI Chat Completions API.
func (a *Adapter) Query(ctx context.Context, req core.QueryRequest) (<-chan core.StreamEvent, error) {
	if a.client == nil {
		return nil, fmt.Errorf("openai: adapter not initialized, call Init() first")
	}
...
func (a *Adapter) readStream(ctx context.Context, body io.ReadCloser, ch chan<- core.StreamEvent) {
	...
	for {
		...
		if err == io.EOF {
			if !sentDone {
				...
				if ctx.Err() != nil {
					...
					finalErr = fmt.Errorf("openai: stream ended unexpectedly (no [DONE] received)")
				}
		...
		if err != nil {
			...
			trySend(ch, &providers.ErrorEvent{Err: fmt.Errorf("openai: stream parse error: %w", err)})
			return
		}
		if event != nil {
