...
// tier3-slow-plugin sleeps 60s on Execute to test timeout behavior.
package main
...
func (s *pluginServer) Execute(ctx context.Context, _ *siplyv1.ExecuteRequest) (*siplyv1.ExecuteResponse, error) {
	// Respect context cancellation for graceful timeout handling (P15).
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
